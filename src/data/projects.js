import ecommerce from "../assets/images/ecommerce.png";
import ecommerce1 from "../assets/images/ecommerce1.png";
import ecommerce2 from "../assets/images/ecommerce2.png";
import ecommerce3 from "../assets/images/ecommerce3.png";
import ecommerce4 from "../assets/images/ecommerce4.png";
import ecommerce5 from "../assets/images/ecommerce5.png";
import ecommerce6 from "../assets/images/ecommerce6.png";
import ecommerce7 from "../assets/images/ecommerce7.png";
import ecommerce8 from "../assets/images/ecommerce8.png";
import ecommerce9 from "../assets/images/ecommerce9.png";
import ecommerce10 from "../assets/images/ecommerce10.png";
// film
import film from "../assets/images/film.png";
import film1 from "../assets/images/film1.png";
import film2 from "../assets/images/film2.png"; 
import film3 from "../assets/images/film3.png";
import film4 from "../assets/images/film4.png";
import film5 from "../assets/images/film5.png";
import film6 from "../assets/images/film6.png";
// landing
import landing from "../assets/images/landing.png";
import landing1 from "../assets/images/landing1.png";
import landing2 from "../assets/images/landing2.png";
// pizza
import pizza from "../assets/images/pizza.png";
import pizza1 from "../assets/images/pizza1.png";
import pizza2 from "../assets/images/pizza2.png";
// pags
import pags from '../assets/images/pags.png';
import pags1 from '../assets/images/pags1.png';
import pags2 from '../assets/images/pags2.png';
import pags3 from '../assets/images/pags3.png';
import pags4 from '../assets/images/pags4.png';
import pags5 from '../assets/images/pags5.png';
import pags6 from '../assets/images/pags6.png';
// watch
import watch from '../assets/images/watch.png';
import watch1 from '../assets/images/watch1.png';
import watch2 from '../assets/images/watch2.png';
import watch3 from '../assets/images/watch3.png';
import watch4 from '../assets/images/watch4.png';
import watch5 from '../assets/images/watch5.png';
// cloth
import cloth from '../assets/images/cloth.png';
import cloth1 from '../assets/images/cloth1.png';
import cloth2 from '../assets/images/cloth2.png';
import cloth3 from '../assets/images/cloth3.png';
import cloth4 from '../assets/images/cloth4.png';
import cloth5 from '../assets/images/cloth5.png';
// perfume
import perfume from '../assets/images/perfume.png';
import perfume1 from '../assets/images/perfume1.png';
import perfume2 from '../assets/images/perfume2.png';
import perfume3 from '../assets/images/perfume3.png';
import perfume4 from '../assets/images/perfume4.png';
import perfume5 from '../assets/images/perfume5.png';
import perfume6 from '../assets/images/perfume6.png';
// buyora
import buyora from '../assets/images/buyora.png';
import buyora1 from '../assets/images/buyora1.png';
import buyora2 from '../assets/images/buyora2.png';
import buyora3 from '../assets/images/buyora3.png';
import buyora4 from '../assets/images/buyora4.png';
import buyora5 from '../assets/images/buyora5.png';
import buyora6 from '../assets/images/buyora6.png';
import buyora7 from '../assets/images/buyora7.png';
import buyora8 from '../assets/images/buyora8.png';
import buyora9 from '../assets/images/buyora9.png';
import buyora10 from '../assets/images/buyora10.png';
import buyora11 from '../assets/images/buyora11.png';
import buyora12 from '../assets/images/buyora12.png';
import buyora13 from '../assets/images/buyora13.png';
import buyora14 from '../assets/images/buyora14.png';

const projects = [
  {
    id: 1,
    title: "E-commerce",
    description: "Online store",
    longDescription: "Full e-commerce app with cart and filtering",
    images: [ecommerce,ecommerce1,ecommerce2,ecommerce3,ecommerce4,ecommerce5,ecommerce6,ecommerce7,ecommerce8,ecommerce9,ecommerce10,],
    demo:'https://fastshop-pearl.vercel.app/',
  },
  {
    id: 2,
    title: "Movie",
    description: "Online Movie Site",
    longDescription: "Smart landing page UI",
    images: [film,film2,film1,film3,film4,film5,film6],
    demo:'https://cimapop.vercel.app/',
  },
  {
    id: 3,
    title: "Landing Page",
    description: "Marketing page",
    longDescription: "Smart landing page UI",
    images: [landing,landing1,landing2],
    // demo:'https://perfume-2fgl.vercel.app/',
  }
  ,
  {
    id: 4,
    title: "Landing Page",
    description: "Marketing page",
    longDescription: "Smart landing page UI",
    images: [pizza,pizza1,pizza2],
    // demo:'https://perfume-2fgl.vercel.app/',
  },
  {
    id: 5,
    title: "Landing Page for bags",
    description: "Marketing page",
    longDescription: "Smart landing page UI",
    images: [pags,pags1,pags2,pags3,pags4,pags5,pags6],
    demo:'https://perfume-2fgl.vercel.app/',
  },
  {
    id: 6,
    title: "Landing Page for watch",
    description: "Marketing page",
    longDescription: "Smart landing page UI",
    images: [watch,watch1,watch2,watch3,watch4,watch5],
    demo:'https://prewatch.vercel.app/',
  },
  {
    id: 7,
    title: "Landing Page for cloth",
    description: "Marketing page",
    longDescription: "Smart landing page UI",
    images: [cloth,cloth1,cloth2,cloth3,cloth4,cloth5],
    demo:'https://cloth-three-weld.vercel.app/',
  },
  {
    id: 8,
    title: "Landing Page for perfume",
    description: "Marketing page",
    longDescription: "Smart landing page UI",
    images: [perfume,perfume1,perfume2,perfume3,perfume4,perfume5,perfume6],
    demo:'https://perfume-rho-ten.vercel.app/',
  },
  {
    id: 9,
    title: "Buyora pro e-commerce",
    description: "e-commerce shop",
    longDescription: "Smart e-commerce ,responsive",
    images: [buyora,buyora1,buyora2,buyora3,buyora4,buyora5,buyora6,buyora7,buyora8,buyora9,buyora10,buyora11,buyora12,buyora13,buyora14,],
    demo:'https://buyora-henna.vercel.app/',
  }
];

export default projects;